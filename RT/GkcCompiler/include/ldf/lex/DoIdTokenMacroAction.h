...
// _I_IdTokenMacroAction_Utility methods
	virtual void SetOutput(const GKC::RefPtr<GKC::TokenTable>& table, const GKC::ShareArray<_LexerTokenString>& arr, const GKC::RefPtr<uint>& uID) throw()
	{
		m_table = table;
		m_arr = arr;
		m_uCurrentID = uID;
...
private:
	...
	ShareArray<_LexerTokenString> m_arr;
	//ID
	uint m_uMinID;
