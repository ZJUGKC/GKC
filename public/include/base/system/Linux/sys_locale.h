...
{
public:
	charset_converter() throw() : m_cvt((iconv_t)-1)
	{
	}
...
//init
	...
	{
		assert( m_cvt == (iconv_t)-1 );
		m_cvt = ::iconv_open("UTF-32", "UTF-16");
		return m_cvt != (iconv_t)-1;
	}
	// utf32->ansi
	...
	{
		assert( m_cvt == (iconv_t)-1 );
		m_cvt = ::iconv_open("UTF-16", "UTF-32");
		return m_cvt != (iconv_t)-1;
	}
...
//convert
	...
	int Convert(const void* pSrc, int iSrcBytes, void* pDest, int iDestBytes) throw()
	{
		...
		::iconv(m_cvt, NULL, NULL, NULL, NULL);
		//convert
		char*  ps = (char*)pSrc;
		size_t s_bytes = (size_t)iSrcBytes;
		char*  pd = (char*)pDest;
		size_t d_bytes = (size_t)iDestBytes;
		size_t res = ::iconv(m_cvt, &ps, &s_bytes, &pd, &d_bytes);
		if( res == (size_t)-1 ) {
			if( errno == E2BIG )
				return 0;
			return -1;
